...
| ADR-013 | **Anonymní účet smí zakládat konverzace i pozvánky** (ruší omezení z 27 §Omezení anonymních účtů). Důvod: e-mailová brána je slabá zábrana (throwaway e-mail obchází kohokoli motivovaného) a zároveň odrazuje privacy-first cílovku (vnímá to jako riziko spamu). Ochrana proti zneužití se přesouvá na **App Check + rate limity per zařízení/IP** (míří na skutečné riziko — hromadné automatizované generování odkazů coby phishing payload jinde) + existující report/block. E-mail zůstává **volitelný** upgrade (obnova identity, nalezitelnost Známými). Přiznané zbytkové riziko: triviální ban-evasion (26 — raději přiznat mez). App Check se zapne s Blaze (45) | 27, `functions/src/spaces.ts`, `functions/src/invites.ts` |
| ADR-014 | **Rychlé místnosti — životní cyklus:** (a) nepřečtená zpráva vyprší default za **1 h** (dřív 24 h; koridor v Rules uvolněn na (now, 24 h] — klient/admin může nastavit kratší i delší až ke stropu 24 h; kratší je vždy bezpečné, fail-secure 34 §5). (b) **Odchod účastníka = zánik celé místnosti** (`leaveSpace` CF smaže zprávy, payloady, členy, pozvánky i Space). Konzistentní s „obsah prchavý": kdokoli může ukončit sdílený ephemeral moment pro všechny. Vědomá vlastnost, ne bug (u vícečlenného Space to znamená „kdokoli ukončí pro všechny") | 34, 11, `functions/src/spaces.ts`, `infra/firestore.ephemeral.rules` |
